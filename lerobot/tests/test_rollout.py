...
def test_rollout_top_level_imports():
    import lerobot.src.lerobot.rollout

    for name in lerobot.src.lerobot.rollout.__all__:
        assert hasattr(lerobot.src.lerobot.rollout, name), f"Missing export: {name}"


def test_inference_submodule_imports():
    import lerobot.src.lerobot.rollout.inference

    for name in lerobot.src.lerobot.rollout.inference.__all__:
        assert hasattr(lerobot.src.lerobot.rollout.inference, name), f"Missing export: {name}"


def test_strategies_submodule_imports():
    import lerobot.src.lerobot.rollout.strategies

    for name in lerobot.src.lerobot.rollout.strategies.__all__:
        assert hasattr(lerobot.src.lerobot.rollout.strategies, name), f"Missing export: {name}"
...
def test_strategy_config_types():
    from lerobot.src.lerobot.rollout import (
        BaseStrategyConfig,
...
def test_dagger_config_invalid_input_device():
    from lerobot.src.lerobot.rollout import DAggerStrategyConfig
...
def test_inference_config_types():
    from lerobot.src.lerobot.rollout import RTCInferenceConfig, SyncInferenceConfig
...
def test_sentry_config_defaults():
    from lerobot.src.lerobot.rollout import SentryStrategyConfig
...
def test_ring_buffer_append_and_eviction():
    from lerobot.src.lerobot.rollout.ring_buffer import RolloutRingBuffer
...
def test_thread_safe_robot_delegates():
    from lerobot.src.lerobot.rollout.robot_wrapper import ThreadSafeRobot
    from lerobot.tests.mocks.mock_robot import MockRobot, MockRobotConfig
...
def test_create_strategy_unknown_raises():
    from lerobot.src.lerobot.rollout import create_strategy
...
def test_create_inference_engine_sync():
    from lerobot.src.lerobot.rollout import SyncInferenceConfig, SyncInferenceEngine, create_inference_engine
...
def test_estimate_max_episode_seconds_no_video():
    from lerobot.src.lerobot.rollout.strategies import estimate_max_episode_seconds
...
def test_safe_push_to_hub():
    from lerobot.src.lerobot.rollout.strategies import safe_push_to_hub
...
def test_dagger_full_transition_cycle():
    from lerobot.src.lerobot.rollout.strategies import DAggerEvents, DAggerPhase
...
def test_rollout_context_fields():
    from lerobot.src.lerobot.rollout import RolloutContext
